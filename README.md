...
If you’re working outside of the locations folder, you can speed things up even more by using the dev config file…

```jekyll build && jekyll serve --config _config_dev.yml --incremental```

The dev config file doesn’t generate any of the files in these folders…
...
These markdown files were initially generated by [Node.js](https://nodejs.org) from the data files in the `_data` folder.
